import { ProductType } from "../context/ProductsProvider"
import { ReducerActionType, ReducerAction  } from "../context/CartProvider"
import { ReactElement, memo } from "react"

type PropsType = {
    product: ProductType,
    dispatch: React.Dispatch<ReducerAction>,
    REDUCER_ACTIONS: ReducerActionType,
    inCart: boolean
}

function Product({ product, dispatch, REDUCER_ACTIONS, inCart }: PropsType): ReactElement {

    //const img: string = require(`../images/${product.sku}.jpg`) // old way, won't work with Vite

    const img: string = new URL(`../images/${product.sku}.jpg`, import.meta.url).href
    console.log(img)

    const onAddToCard = () => dispatch({type: REDUCER_ACTIONS.ADD, payload: {...product, qty: 1}})
    const itemInCart = inCart ? ' → Item in Cart: ✔️' : null

    const content = 
        <article className="product">
            <h3>{product.name}</h3>
            <img className="product__img" src={img} alt={product.name} />
            <p>{new Intl.NumberFormat("en-AU", { style: "currency", currency: "AUD" }).format(product.price)}{itemInCart}</p>
            <button onClick={onAddToCard}>Add to cart</button>
        </article>  

    return content
}

/* export default Product */

const areProductsEqual = ({ product: prevProduct, inCart: prevInCart }: PropsType, {product: nextProduct, inCart: nextInCart }: PropsType): boolean => {
    return (
        Object.keys(prevProduct).every(key => {
        return prevProduct[key as keyof ProductType] === nextProduct[key as keyof ProductType]
        }) && prevInCart === nextInCart
    )
}

const MemoizedProduct = memo<typeof Product>(Product, areProductsEqual)

export default MemoizedProduct